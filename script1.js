...
function updateAvailableJobsCount() {
    ...
    if (btnAllFilter.classList.contains('bg-blue-500')) {
        ...
        counterElement.innerHTML =
            `<p><span>${total} Total Jobs (${active} Active, ${interviewList.length} Interview, ${rejectedList.length} Rejected)</span></p>`;
    }
    else if (btnInterviewFilter.classList.contains('bg-blue-500')) {
        counterElement.innerHTML =
            `<p><span>${interviewList.length} of ${total} Jobs</span></p>`;
    }
    else if (btnRejectedFilter.classList.contains('bg-blue-500')) {
        counterElement.innerHTML =
            `<p><span>${rejectedList.length} of ${total} Jobs</span></p>`;
    }
}
...
mainContainer.addEventListener('click', function (event) {
    ...
    if (event.target.classList.contains('btn-interview')) {
        const jobCard = event.target.closest('.flex.justify-between');
        if (!jobCard) return;
    ...
    if (event.target.classList.contains('fa-trash-can') || event.target.classList.contains('fa-regular')) {
        ...
        const jobCard = deleteButton.closest('.flex.justify-between');
        if (!jobCard) return;
